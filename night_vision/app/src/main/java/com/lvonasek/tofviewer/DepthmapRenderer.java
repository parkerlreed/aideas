...
class DepthmapRenderer implements ImageReader.OnImageAvailableListener {
  ...
  public void onImageAvailable(ImageReader imageReader) {
    ...
    ShortBuffer shortDepthBuffer = planeBuffer.asShortBuffer();
   ...
   */
  synchronized void draw(Activity context, float zoom, int viewportWidth, int viewportHeight) {
    ...
    if (rotation == Surface.ROTATION_90 || rotation == Surface.ROTATION_270) {
      // shader passes through x/y unchanged: depthWidth fills horizontal, depthHeight fills vertical
      x = viewportWidth / (float) depthWidth;
      y = viewportHeight / (float) depthHeight;
    } else {
      // shader swaps x/y: depthHeight fills horizontal, depthWidth fills vertical
      x = viewportWidth / (float) depthHeight;
      y = viewportHeight / (float) depthWidth;
    }
    ...
    GLES20.glUniform1f(pointSizeUniform, (float) Math.ceil(Math.max(x, y) * getStep() * zoom));
